import { Scene, PerspectiveCamera, WebGLRenderer, Vector3, PointLight, HemisphereLight, DirectionalLight, SRGBColorSpace, Mesh, MeshStandardMaterial, Group } from "three";
...
export class OfficeArena extends Arena {
    ...
    private targetCamera: Vector3 | undefined;
    needsLoading = true;
    focus = false;
    ...
    office = (scene: Scene): void => {
        this.loader.load('/models/living_room.glb', function (gltf) {
    ...
    sky = (scene: Scene): void => {
        ...
        this.loader.load('/models/hdSky.glb', function (gltf) {
    ...
    update = (): void => {
        ...
        if (!this.focus) {
            ...
            (this.targetCamera as Vector3).x += (this.mouse.percentage.x * 5 - (this.targetCamera as Vector3).x) / 500;
            (this.targetCamera as Vector3).y += (-(this.mouse.percentage.y * 5) - (this.targetCamera as Vector3).y) / 500;
            // this.targetCamera.y += (-(this.mouse.percentage.y *5) + 40 - this.targetCamera.y) / 10;

            this.camera?.lookAt((this.targetCamera as Vector3));
        }
    };
